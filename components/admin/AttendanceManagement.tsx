    ...
    return (
      <div className="p-8 space-y-6">
        ...
        <div className="bg-[#5e3ed0]/20 rounded-xl border border-white/10 overflow-hidden backdrop-blur-md">
          <div className="p-4 border-b border-white/10"><h3 className="font-semibold text-white">Lịch sử chấm công</h3></div>
          {userRecords.length === 0 ? (
            <div className="p-8 text-center text-slate-300">Chưa có dữ liệu chấm công</div>
          ) : (
                ...
                <div key={record.id} className="p-4 hover:bg-white/5 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-white">{new Date(record.date).toLocaleDateString('vi-VN', { weekday: 'long', day: 'numeric', month: 'numeric', year: 'numeric' })}</p>
                        <p className="text-sm text-slate-400">Check-in: {record.checkInTime?.toLocaleTimeString('vi-VN')}{record.checkOutTime && ` | Check-out: ${record.checkOutTime.toLocaleTimeString('vi-VN')}`}{record.workHours && ` | ${record.workHours} giờ`}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-4 ml-16">
                      {record.checkInPhoto && (
                        <div className="text-center">
                          ...
                          />
                          <p className="text-xs text-slate-400 mt-1">Check-in</p>
                        </div>
                      )}
                      {record.checkOutPhoto && (
                        <div className="text-center">
                          ...
                          />
                          <p className="text-xs text-slate-400 mt-1">Check-out</p>
                        </div>
                      )}
                    </div>
                  )}
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white">Quản lý chấm công</h2>
      ...
      {isManager && managedDepartment && (
        <div className="bg-purple-500/20 border border-purple-500/30 rounded-lg p-4 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-white">Trưởng phòng {managedDepartment.name}</p>
              <p className="text-sm text-purple-200">Bạn chỉ có thể xem chấm công của nhân viên trong phòng ban mình</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-[#53cafd]/10 border border-[#53cafd]/30 rounded-lg p-4 backdrop-blur-md">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="font-medium text-white">Cài đặt công ty</p>
              <p className="text-sm text-slate-300 mt-1">Giờ làm: {companySettings.workStartTime} - {companySettings.workEndTime} | Ngày công: {companySettings.workingDaysPerMonth} | IP: {companySettings.allowedIPs?.length || 0} địa chỉ</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium text-slate-300">Trạng thái:</label>
            <select
              value={filterStatus}
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-[#5e3ed0]/20 p-4 rounded-lg border border-white/10 backdrop-blur-md">
          <p className="text-sm text-slate-300">Đã chấm công hôm nay</p>
        ...
        <div className="bg-[#5e3ed0]/20 p-4 rounded-lg border border-white/10 backdrop-blur-md">
          ...
          <p className="text-2xl font-bold text-[#53cafd]">{filteredUsers.reduce((sum, u) => sum + (u.monthlySalary || 0), 0).toLocaleString('vi-VN')}đ</p>
        </div>
        <div className="bg-[#5e3ed0]/20 p-4 rounded-lg border border-white/10 backdrop-blur-md">
      ...
      <div className="bg-[#5e3ed0]/20 rounded-xl border border-white/10 overflow-hidden backdrop-blur-md">
        <table className="w-full">
          <thead className="bg-[#5e3ed0]/40 border-b border-white/10">
            <tr>
              ...
              <th className="px-6 py-3 text-center text-xs font-medium text-slate-300 uppercase">Hôm nay</th>
              <th className="px-6 py-3 text-center text-xs font-medium text-slate-300 uppercase">Ngày đi làm</th>
              <th className="px-6 py-3 text-center text-xs font-medium text-slate-300 uppercase">Tổng giờ làm</th>
              <th className="px-6 py-3 text-center text-xs font-medium text-slate-300 uppercase">Tổng phút muộn</th>
                ...
                <tr key={user.uid} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    {todayRecord ? (
                      ...
                      <div className="text-center text-xs text-slate-500">Chưa chấm công</div>
                    )}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right font-medium text-white">{(user.monthlySalary || 0).toLocaleString('vi-VN')}đ</td>
                  <td className="px-6 py-4 text-right text-green-400 font-bold">{(savedSalary?.finalSalary || salary?.finalSalary || 0).toLocaleString('vi-VN')}đ</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    ...
                    {isAdmin && <Button onClick={() => handleSaveSalary(user.uid)} className="px-3 py-1.5 text-sm bg-[#53cafd] hover:bg-[#3db9f5] border-none text-white shadow-[#53cafd]/25" disabled={!salary}>{savedSalary ? 'Cập nhật' : 'Tính lương'}</Button>}
                  </td>
                </tr>
              );
            })}
        ...
        {filteredUsers.length === 0 && (
          <div className="p-8 text-center text-slate-300">Không có nhân viên nào</div>
        )}
      </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#311898]/90 backdrop-blur-xl border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="flex justify-between items-center p-6 border-b border-white/10">
          <h3 className="text-xl font-bold text-white">Cài đặt công ty</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-semibold text-white flex items-center gap-2"><Wifi size={20} className="text-[#53cafd]" /> Địa chỉ IP công ty</h4>
            ...
            <div className="bg-white/5 rounded-lg p-4 mb-4 border border-white/10">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-slate-300">IP hiện tại của bạn</p>
                  <p className="font-mono text-lg font-bold text-white">{loadingIP ? 'Đang tải...' : currentIP || 'Không xác định'}</p>
                </div>
            ...
            {showBulkInput && (
              <div className="bg-white/5 rounded-lg p-4 mb-4 border border-[#53cafd]/30">
                <p className="text-sm font-medium text-slate-300 mb-2">Nhập nhiều IP (mỗi IP một dòng)</p>
                <textarea
                  value={bulkIPText}
            ...
            <div className="space-y-2">
              <p className="text-sm font-medium text-slate-300">Danh sách IP được phép ({ipList.length})</p>
              {ipList.length === 0 ? (
                <div className="text-center py-6 text-slate-500 bg-white/5 rounded-lg border border-dashed border-white/10">Chưa có IP nào</div>
              ) : (
                  ...
                  {ipList.map((ip, index) => (
                    <div key={index} className="flex items-center justify-between bg-white/5 px-4 py-3 rounded-lg border border-white/10">
                      <div className="flex items-center gap-3">
                        ...
                        {ip === currentIP && <span className="text-xs bg-[#53cafd]/20 text-[#53cafd] px-2 py-0.5 rounded-full">IP hiện tại</span>}
                      </div>
          ...
          {/* Work Hours */}
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <h4 className="font-semibold text-white mb-4 flex items-center gap-2"><Clock size={20} className="text-[#53cafd]" /> Giờ làm việc</h4>
          ...
          {/* Other Settings */}
          <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            <h4 className="font-semibold text-white mb-4 flex items-center gap-2"><Settings size={20} className="text-[#53cafd]" /> Cài đặt khác</h4>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Cho phép đi muộn (phút)</label>
                ...
                <p className="text-xs text-slate-400 mt-1">Số phút được phép đến muộn</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Số ngày công/tháng</label>
                <input type="number" min="1" max="31" value={workingDays} onChange={(e) => setWorkingDays(Number(e.target.value))}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#53cafd] text-white" />
                <p className="text-xs text-slate-400 mt-1">Số ngày công chuẩn để tính lương</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-white/10 flex gap-3">
          <Button onClick={handleSubmit} className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#53cafd] hover:bg-[#3db9f5] border-none text-white shadow-[#53cafd]/25"><Save size={18} /> Lưu cài đặt</Button>
          <button onClick={onClose} className="flex-1 px-4 py-3 border border-white/10 rounded-lg hover:bg-white/10 font-medium text-white transition-colors">Hủy</button>
        </div>
      </div>
    </div>
  );
